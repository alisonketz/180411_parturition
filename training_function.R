        ...
        for(j in 1:nInd.train){
          ...
          for(h in 1:nCovs){
            for(k in 1:epsNum){# loop over epsilon values
              ...
              # fit.train = anomalyDetect(n.vit=n.loo,id=individs,d=df[,c(1:3,h+3)],eps=k/100,covs.indx = 4:(3+nCovs))# fit model
          ...
          fit.test= training(d=d.ind,pw=part.window,eps=decide[,m],vd=vdrop[j])# fit model
